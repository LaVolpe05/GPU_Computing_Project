...
int main( void)
{
	...
	if ( -1 == ( msqid = msgget( (key_t)12345, IPC_CREAT | 0666)))
	{
	...
	while( 1 )
	{
		...
		else if (data.data_type == TYPE_TWODOUBLES)
		{
			...
			printf("Interpreted as two int: %d, %d\n", (int)dValue1, (int)dValue2);
			data.size_values_array =(int) dValue1;
			
			int pointeur = (int)dValue1;
			int size = (int)dValue2;
		...
		{	
			...
			int values[data.size_values_array];
			printf("Interpreted as array: ");
			int i;
			for(i = 0; i<BUFF_SIZE; i++)
				if(i<data.size_values_array){
				//printf("%d ", data.data_buff[i]);
				values[i] = (int)data.data_buff[i];
				}
			printf("\n");
		}
		else if (data.data_type == TYPE_DOUBLEANDARRAY)
		{
			...
			printf("Interpreted as one double and array: %f\n", dValue1);
			int i;
